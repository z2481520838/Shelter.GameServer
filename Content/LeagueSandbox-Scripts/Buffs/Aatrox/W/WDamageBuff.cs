    ...
    class AatroxWPower : IBuffGameScript
    {
        ...
        public void OnUpdate(float diff)
        {
            if (owner != null && spell != null && Unit != null)
            {
                ...
                if (buff != null && buff.StackCount == buff.MaxStacks && !Unit.HasBuff("AatroxWONHPowerBuff"))
                {
                    //RemoveBuff(Unit, "AatroxWLife");
                    //RemoveBuff(Unit, "AatroxWONHLifeBuff");
                    AddBuff("AatroxWONHPowerBuff", float.MaxValue, 1, spell, Unit, owner, false);
                    LogInfo("testeDano");
                }
            }
        }
